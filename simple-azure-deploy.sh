...
cp production-server.js simple-deploy/server.js

# Create proper package.json
cat > simple-deploy/package.json << 'PKGJSON'
{
  "name": "cai-news-app",
  "version": "1.0.0",
...
cp -r frontend/dist/* simple-deploy/public/

# Create startup command file
echo "node server.js" > simple-deploy/startup.txt

# Update server.js to serve static files
cat > simple-deploy/server.js << 'SERVERJS'
...
# Create zip
cd simple-deploy
zip -r ../cai-news-simple.zip . > /dev/null
cd ..
...
echo ""
echo "🌐 Your app: https://cai-news-cwb3aebqdze3d2f0.westus-01.azurewebsites.net"
echo ""
echo "⚙️  Add your OpenAI key:"
echo "az webapp config appsettings set --resource-group $RESOURCE_GROUP --name $APP_NAME --settings OPENAI_API_KEY=your_key"
